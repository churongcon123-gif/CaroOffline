...
const API_URL = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/game` : 'http://localhost:5000/api/game';
...
export const getLeaderboardApi = async (limit = 50) => {
  try {
    const url = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/leaderboard?limit=${limit}` : `http://localhost:5000/api/leaderboard?limit=${limit}`;
